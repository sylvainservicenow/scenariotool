    ...
    >
      <div className="step-card-top">
        ...
        <div className="step-badges">
          {step.liveStatus && statusLabels[step.liveStatus] && (
            <span className="badge badge-status" style={{ background: statusLabels[step.liveStatus].color }}>
              {statusLabels[step.liveStatus].label}
            </span>
          )}
      ...
      {step.persona && (
        <div className="step-persona">
          ...
          {step.personaNote && (
            <span className="step-persona-note"> — {step.personaNote}</span>
          )}
        </div>
      )}
      ...
      {expanded && (
        <div className="step-expanded">
          ...
          {step.outputs && step.outputs.length > 0 && (
            <div>
              <div className="step-section-label">Outputs</div>
              <div className="output-pills">
                {step.outputs.map((o, i) => (
                  <span key={i} className="output-pill">{o}</span>
                ))}
              </div>
            </div>
          )}
          ...
          {step.dataSources && step.dataSources.length > 0 && (
            <div>
              <div className="step-section-label">Data Sources</div>
              <div className="data-sources">
                {step.dataSources.map((ds, i) => (
                  <span key={i} className={'data-source' + (isMock(ds) ? ' mock' : '')}>
                    {ds}
                  </span>
                ))}
              </div>
          ...
          {step.url && (
            <div>
              ...
              <span className="url-print-text">{step.url}</span>
            </div>
          )}
        </div>
      )}
