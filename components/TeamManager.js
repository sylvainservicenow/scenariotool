...
export default function TeamManager({ team, onClose, onDeleteTeam }) {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      const { data, error } = await sb.from('team_members').select('user_id, role').eq('team_id', team.id);
      if (error) { console.warn('Members fetch:', error); setMembers([{ userId: user?.id, role: team.role, displayName: user?.user_metadata?.display_name || 'You' }]); setLoading(false); return; }
      const list = [];
      for (const m of (data || [])) {
        const { data: p } = await sb.from('profiles').select('display_name').eq('id', m.user_id).single();
        list.push({ userId: m.user_id, role: m.role, displayName: p?.display_name || 'Unknown' });
      }
  ...
  const handleDeleteTeam = async () => {
    ...
    try {
      ...
      if (error) { alert('Failed: ' + error.message); setDeleting(false); return; }
      if (onDeleteTeam) onDeleteTeam(team.id);
      onClose();
    } catch (e) { console.error('Delete team:', e); setDeleting(false); }
  };
        ...
        <div className="modal-body">
          ...
          {members.map(m => (
            <div key={m.userId} className="member-row">
              ...
              <div className="member-info"><div className="member-name">{m.displayName}{m.userId === user?.id && <span style={{ fontSize: 11, color: 'var(--text-muted)', marginLeft: 6 }}>(you)</span>}</div><div className="member-role">{m.role}</div></div>
              {isOwner && m.userId !== user?.id && <div className="member-actions">
          ...
          {isOwner && <div style={{ marginTop: 24, paddingTop: 16, borderTop: '1px solid var(--border-subtle)' }}>
            <div style={{ fontSize: 12, fontWeight: 600, color: 'var(--text-muted)', marginBottom: 8, letterSpacing: 0.5, textTransform: 'uppercase' }}>Danger zone</div>
            <div style={{ fontSize: 13, color: 'var(--text-secondary)', marginBottom: 10 }}>Deleting removes all members. Linked scenarios become personal.</div>
            <button className="panel-btn" style={confirmDeleteTeam ? { background: 'var(--sn-orange)', color: 'white', borderColor: 'var(--sn-orange)' } : { color: 'var(--sn-orange)', borderColor: 'rgba(207,74,0,0.3)' }}
              onClick={handleDeleteTeam} disabled={deleting}>{deleting ? 'Deleting...' : confirmDeleteTeam ? 'Confirm deletion' : 'Delete team'}</button>
          </div>}
        </div>
