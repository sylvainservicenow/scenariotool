...
function ToolContent() {
    ...
    const load = async () => {
      if (urlId) {
        try {
          ...
          console.error('Scenario load exception:', e);
  ...
  const saveToDb = useCallback(async (d) => {
    ...
    try {
      const { error } = await getSupabase().from('scenarios')
        .update({ title: d.title || 'Untitled', subtitle: d.subtitle || '', data: d })
        .eq('id', scenarioId);
      setSaveStatus(error ? 'error' : 'saved');
      if (error) console.warn('Auto-save error:', error);
    } catch (e) { setSaveStatus('error'); console.error('Auto-save exception:', e); }
  }, [scenarioId, canEdit]);
  ...
  const handleSaveAsNew = useCallback(async () => {
    ...
    try {
      const { data, error } = await getSupabase().from('scenarios')
        ...
        .select().single();
      if (error) { console.error('Save as new error:', error); alert('Save failed: ' + error.message); return; }
  ...
  const handleNewScenario = useCallback(async () => {
    ...
    try {
      const { data, error } = await getSupabase().from('scenarios')
        ...
        .select().single();
      if (error) { console.error('New scenario error:', error); alert('Create failed: ' + error.message); return; }
      if (data) { setDrawerOpen(false); router.push('/tool/' + data.id); }
    } catch (e) { console.error('New scenario exception:', e); }
  }, [user, router]);
  ...
  const startPres = () => {
    ...
    setPresStep(0); setPresenting(true); setPanelOpen(false); setDrawerOpen(false);
  };
  ...
  if (!scenario) {
    return <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: '100vh', color: 'var(--text-muted)', fontSize: 15 }}>Loading scenario...</div>;
  }
  ...
  return (
    <div>
      ...
      <Header
        scenario={scenario} isSample={isSample} canEdit={canEdit} saveStatus={saveStatus}
